...
public class ArabicWriter : EditorWindow
{
    ...
    void OnGUI(){
      ...
      {_OnlySideMenu = GUILayout.Toggle(_OnlySideMenu,new GUIContent(EditorGUIUtility.IconContent("d_UnityEditor.SceneHierarchyWindow@2x").image,"Show side menu"), "toolbarButton", GUILayout.Width(40));
      }
      else
      {
        _OnlySideMenu = false;
...
{ 
       ...
       if (GUILayout.Button(new GUIContent(EditorGUIUtility.IconContent("Linked@2x").image,"Links"), "toolbarButton", GUILayout.Width(40)))
        {
            ...
            LinksMenu.AddItem(new GUIContent("• Discord"), false, Link2);
            ...
            LinksMenu.AddItem(new GUIContent("★★ Rate this Tool ★★"), false, Link5);
            LinksMenu.AddItem(new GUIContent(""), false, slot4);
            LinksMenu.AddItem(new GUIContent("Email ✉"), false, Link6);

            LinksMenu.ShowAsContext();
        }
      GUIContent iconContent = EditorGUIUtility.IconContent("d_TrueTypeFontImporter Icon");
      GUILayout.Label(iconContent, GUILayout.Width(19), GUILayout.Height(19));  
      float tempSliderValue = GUILayout.HorizontalSlider(sliderValue, minValue, maxValue, GUILayout.Width(75));
      sliderValue = Mathf.RoundToInt(tempSliderValue);
      
      GUILayout.Label("" + sliderValue);   
      GUI.skin.label.alignment = TextAnchor.MiddleRight;
      GUILayout.Label("v2.2");
    ...
    private void Tab1()
    {
      ...
      EditorGUILayout.BeginVertical();
        ...
        myStyle.fontSize = sliderValue;
        
        GUIStyle myStyle2 = new GUIStyle(GUI.skin.textField);
        myStyle2.fontSize = sliderValue;
        myStyle.alignment = TextAnchor.UpperRight;
        InputText = EditorGUILayout.TextArea(InputText,myStyle2,GUILayout.MinHeight(64),GUILayout.ExpandHeight(true));
        ...
        if (GUILayout.Button(new GUIContent("Copy","Copy the correct text"), buttonStyle))
        {
          GUIUtility.systemCopyBuffer = OutputText;
        }
        GUI.backgroundColor = Color.red;
       if( GUILayout.Button(new GUIContent("Delete","Erase the written"), buttonStyle)){
    ...
    {
      GUILayout.BeginVertical(EditorStyles.centeredGreyMiniLabel);
      GUIContent TrashIcon = EditorGUIUtility.IconContent("d_TreeEditor.Trash");
    ...
    private void Link2()
    {
      Application.OpenURL("https://discord.gg/xaKv2qQ49Q");
    }
    private void Link3()
    {
    ...
    private void Tab_Text()
    {
      ...
      if (GUILayout.Button(new GUIContent("▪","Black Small Square"),"ButtonMid"))
        {
          GUIUtility.systemCopyBuffer = "▪";
        }
      if (GUILayout.Button(new GUIContent("°","Degree Sign"),"ButtonMid"))
        {
          GUIUtility.systemCopyBuffer = "°";
        }
      if (GUILayout.Button(new GUIContent("←","Leftwards Arrow"),"ButtonRight"))
        {
          GUIUtility.systemCopyBuffer = "←";
        }    
      GUILayout.EndHorizontal();
      ...
      if (GUILayout.Button(new GUIContent("®","Registered Sign"),"ButtonMid"))
        {
          GUIUtility.systemCopyBuffer = "®";
        }
      if (GUILayout.Button(new GUIContent("℗","Sound Recording Copyright"),"ButtonMid"))
        {
          GUIUtility.systemCopyBuffer = "℗";
        }
      if (GUILayout.Button(new GUIContent("™","Trade Mark Sign"),"ButtonRight"))
        {
          GUIUtility.systemCopyBuffer = "™";
        }    
      GUILayout.EndHorizontal();
      ...
      if (GUILayout.Button(new GUIContent("€","Euro Sign"),"ButtonMid"))
        {
          GUIUtility.systemCopyBuffer = "€";
        }
      if (GUILayout.Button(new GUIContent("ﷺ","Peace be upon him"),"ButtonMid"))
        {
          GUIUtility.systemCopyBuffer = "ﷺ";
        }
      if (GUILayout.Button(new GUIContent("﷼","Rial Sign"),"ButtonRight"))
        {
          GUIUtility.systemCopyBuffer = "﷼";
        }    
      GUILayout.EndHorizontal();    
    ...
    private void Tab_Info()
    {
      ...
      GUILayout.Label("Arabic Writer is a lightweight tool that enables you to type Arabic text accurately and effortlessly, offering a variety of features to enhance your writing experience!",EditorStyles.label);
      EditorGUILayout.Space(15); 
      ...
      if (_Foldout2)
      {
        GUILayout.Label("An award-winning Saudi-based game studio | Since 2016",EditorStyles.label);
      } 
      ...
      private void Tab_TextGenerator()
    {
      ...
      GUILayout.Label("0600-06FF,0750-077F,08A0-08FF,FB50-FDFF,FE70-FEFF,10E60-10E7F,1EE00-1EEFF,0030-0039,00D7,00F7,002B,2212,003D","TextField",GUILayout.Height(position.height / 5.9f));
      if (GUILayout.Button("Copy"))
        {
          GUIUtility.systemCopyBuffer = "0600-06FF,0750-077F,08A0-08FF,FB50-FDFF,FE70-FEFF,10E60-10E7F,1EE00-1EEFF,0030-0039,00D7,00F7,002B,2212,003D";
        }
        EditorGUILayout.Space(15); 
      EditorStyles.label.wordWrap = true;
